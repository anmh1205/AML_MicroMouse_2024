...
int fastRunFlag = 0;    // "boolean" that stores whether the mouse should run the fastRunSolver
...
void firstFastRun()
{
    ...
    while (getReachingCenter() == 1)
    {
        Action nextMove = firstFastRunSolver();
...
{
    int turn_count = 0;
    queue turn = queue_create();
    queue road = queue_create();

    while (turn_count < 2)
    {
        // if there's no wall to the north && the square to the north hasn't been checked yet
        if (isWallInDirection(x, y, NORTH) == 0 && distances[x][y + 1] > distances[x][y])
        {
            turn_count++;
            queue_push(turn, xyToSquare(x, y + 1));
        }
        // same as ^ but for east
        if (isWallInDirection(x, y, EAST) == 0 && distances[x + 1][y] > distances[x][y])
        {
            turn_count++;
            queue_push(turn, xyToSquare(x + 1, y));
        }
        // same as ^ but for south
        if (isWallInDirection(x, y, SOUTH) == 0 && distances[x][y - 1] > distances[x][y])
        {
            turn_count++;
            queue_push(turn, xyToSquare(x, y - 1));
        }
        // same as ^ but for west
        if (isWallInDirection(x, y, WEST) == 0 && distances[x - 1][y] > distances[x][y])
        {
            turn_count++;
            queue_push(turn, xyToSquare(x - 1, y));
        }
...
int getReachingCenter()
{
    ...
    // if (fastRunFlag > 0 && fastRunFlag % 2 == 0)
    // {
    //     return 0;
    // }
