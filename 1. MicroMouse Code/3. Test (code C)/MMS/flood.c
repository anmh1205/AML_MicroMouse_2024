...
void printFullMazeDistances(struct dist_maze *dm)
{
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 16; j++)
		{
			API_setText(i, j, dm->distance[i][j]);
		}
	}
}

void printFullMazeWalls(struct wall_maze *wm)
{
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 16; j++)
...
void advanceTicksFlood(long ticks, int d, struct coor *c, struct wall_maze *wm)
{
