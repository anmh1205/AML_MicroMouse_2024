...
void readSensor(void)
{
  ...
  while ((micros() - curt) < 60)
    ; // use up time until 60us from where we record curt
  ...
  while ((micros() - curt) < 140)
    ; // 140-60=80us,wait 80us until reflection is gone

  writeRightFront();

  while ((micros() - curt) < 200)
    ; // 200-140=60us, turn on emitter for 60us
...
void about_face() // because, why not?
{
  ...
  while (R_encoder_val - value < ABOUT_FACE_COUNT)
    ; // *********increase value to turn more***********
...
void turn_left() // point turn
{
  ...
  // delay(400);
  while (L_encoder_val - encoder_number < TURN_LEFT_COUNT)
    ; // tune this value for complete turn ************* ///////////////////
...
void visit_node(Maze *this_maze, Stack *this_stack, short x, short y, short flag)
{
  ...
  // so sanh huong di cua chuot va phia cua tuong de set wall theo huong ban do
  if (direction == NORTH)
  {
